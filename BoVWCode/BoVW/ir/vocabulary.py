...
class Vocabulary:
	...
	def fit(self, numClusters, samplePercent, randomState=None):
		...
		clt = MiniBatchKMeans(n_clusters=numClusters, random_state=randomState,
				init='random', init_size=int(numClusters*2), max_iter=50000, max_no_improvement=5000,
				n_init=int(max(5,numClusters/100)), reassignment_ratio=0.01, tol=0.001, verbose=0)
		clt.fit(data)
